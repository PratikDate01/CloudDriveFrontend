...
export const SignInModal: React.FC<SignInModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    setApiError('');
    try {
      // Close the modal first before redirecting
      onClose();
      // googleLogin() will redirect to Google OAuth, which will handle the redirect back
      await googleLogin();
    } catch (err: any) {
        ...
        <div className="px-6 pb-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`block w-full pl-10 pr-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-sm ${
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white py-2.5 px-4 rounded-lg font-semibold hover:from-blue-700 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transform hover:scale-[1.02] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none text-sm"
            >
              {isLoading ? (
          ...
          <div className="space-y-2">
            ...
            <button className="w-full flex items-center justify-center px-3 py-2 border border-gray-300 rounded-lg text-xs font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors">
